...
export function CardWithLink({ title, subtitle, info, link }: CardWithLinkProps) {
  return (
    <Card className="mt-6 w-64 mr-10">
      <CardBody>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="currentColor"
          className="mb-3 h-8 w-8 text-gray-900"
        >
          <path
            fillRule="evenodd"
        ...
        <ul className="list-disc pl-5 mb-1">
          {info?.map((infoItem, index) => (
            <li key={index} className="text-gray-500 text-sm">{infoItem}</li>
          ))}
        </ul>
      </CardBody>
